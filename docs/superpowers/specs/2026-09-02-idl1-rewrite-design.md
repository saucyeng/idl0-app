...
| D6 | **Sync sources always; sync outputs only when content-addressed by their inputs.** Cheap math cells are lazy. Expensive estimators (the iEKF) are *materialised* once post-import, keyed by hash of inputs, and synced as blobs. Real-time CRDT co-editing is deferred; nothing here precludes it. |
...
- **Rust = numbers, JS = pictures.** No number the sync model depends on is computed in JavaScript. No chart is drawn in Rust (until export returns).
...
| | Contract | Fixes |
|---|---|---|
| C1 | **Session schema** | Arrow schema for `data.parquet`; column metadata keys (`scale`, `offset`, `sample_rate_hz`, `unit`, `source_kind`); file metadata (`session_id`, `timestamp_utc_ms`, `device_id?`, `config_checksum?`, `blob_sha256`, `engine_version`); the `Channel ↔ Arrow` round-trip; `session.json` schema. |
| C2 | **Workbook v3** | `.idl1wb` grammar: front matter keys, fence kinds, math-cell definition syntax (multi-line `name = expr`), constants, inline `${}` semantics, cell identity for diffing; the Properties-form generated Plot subset (what `plotForm` emits and parses). |
...
|---|---|---|---|---|
| **L1** core `store/` | Arrow model ↔ Parquet r/w (C1); CAS blob store; SQLite catalog + rebuild; `session.json` (port of `.idl0w` semantics); track artifact writer; profile/settings persistence; session/lap-level ports from the inventory (gate synthesis, session-wide lap renumbering, lap-distance normalisation, filenames) | C1, C4 | 1 | Round-trip tests on real `.idl0` sessions; catalog rebuild from a scanned tree; CLI `idl-rs import`/`sessions` work |
...
| **L7** device/data/settings UI | Ports of the Device, Data (catalog browser, import), Settings tabs over L4/L5 | L4, L5 | 2 | Feature parity with idl0 for those tabs |
...
| **L11** LAN sync | `axum` server, mDNS, pairing, manifest/blob/workbook endpoints, LWW + conflict copies, auto-trigger, status UI | L1, L4 crate | 2 | Phone → desktop blob sync and desktop → phone workbook sync on a home LAN |
...
Real-time CRDT co-editing (`yrs` on cell text — the only thing the merge rule changes); cloud relay; ECharts/3-D; Framework run-sheet export (`idl-rs export-framework`); video/reels export; iOS background sync; CSV beyond a trivial importer.

## 16. Open items (assigned)

- C1: metadata for non-device sources; whether identical-rate channels from different IMUs share rows or interleave.
- C2: cell identity for diffing and conflict copies; math-mode syntax for CodeMirror.
...
- L7: HRM (`channel_sources/hrm_source.dart` exists in idl0) — confirm whether it is a real input before porting.
