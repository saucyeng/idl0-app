...
- **WiFi OTA push hung on Android — never waited for the bind, then couldn't recover (2026-07-08).** Two linked defects, found and fixed during v0.1.0 hardware bring-up:
  ...
  - **Secondary: even once linked, the first proxy connect could hang.** `LoopbackProxy` dialed the device with `createSocket(host, port)` and **no connect timeout**, so a connect during the residual post-`onAvailable` warmup window blocked indefinitely. Hardened defense-in-depth: the proxy now does a **bounded connect** (2.5 s) + accept/dial logging; `WifiTransfer.pushFirmware` gained a response-timeout backstop (45 s — it was the only transfer method without one); and `RealWifiService.pushFirmware` retries the warmup race (up to 5 attempts, mirroring `getFileList`), retrying only connection/timeout failures, never a device-rejected image (`FirmwarePushException`). A cold attempt reaches the device 0 bytes in, so retrying is safe.
  This hardens the handoff for downloads/config too. **Spec disposition:** no spec change needed — §6.2 already describes the proxy/link; this is internal robustness.
