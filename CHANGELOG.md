...
- **WiFi OTA push (and any first request after the bind) could hang forever on Android (2026-07-08).** The loopback proxy that scopes device HTTP to the AP network (`LoopbackProxy.kt`, §6.2) dialed `192.168.4.1:80` with `createSocket(host, port)` — **no connect timeout**. Android fires `onAvailable` the instant it *associates* with the AP, but the route to the device isn't ready for a second or two; the OTA push is the very first request into that cold window (unlike a download, which `getFileList` warms first with its own retry), so the proxy's device-connect blocked indefinitely and the whole push froze — confirmed on-device (`proxy up …` then infinite silence, no `/ota` line on the firmware side). Fixed defense-in-depth across the three layers that each independently allowed the hang: (1) the proxy now does a **bounded connect** (2.5 s) so a cold attempt fails fast instead of hanging, plus accept/dial logging; (2) `WifiTransfer.pushFirmware` gained a response timeout (45 s backstop) — it was the only transfer method without one; (3) `RealWifiService.pushFirmware` now **retries the warmup race** (up to 5 attempts, mirroring `getFileList`), retrying only connection/timeout failures and never a `FirmwarePushException` (a device-rejected image). A cold attempt reaches the device 0 bytes in (`esp_ota_begin` never runs), so retrying is safe. This hardens the handoff for downloads/config too. **Spec disposition:** no spec change needed — §6.2 already describes the proxy; this is internal robustness.
